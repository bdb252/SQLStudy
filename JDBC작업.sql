...
create table member
(
    /*아이디, 비번, 이름은 문자타입으로 정의. not null로 빈값을 허용하지 않는 컬럼으로 정의.*/
	id VARCHAR2(30) NOT NULL,
	pass VARCHAR2(40) NOT NULL,
	name VARCHAR2(50) NOT NULL,
    /*날짜타입으로 선언. null을 허용하는 컬럼으로 정의. 입력값이 없는 경우 현재시각을 디폴트로 입력한다.*/
	regidate DATE DEFAULT SYSDATE,
    /*아웃라인방식으로 아이디 컬럼을 PK로 지정*/
	PRIMARY KEY (id)
);
...
commit;
/*
    더미데이터 입력 후 commit을 실행하지 않으면 오라클 외부 프로그램에서는 새롭게 입력한 데이터를 확인할 수 없다.
    입력된 레코드를 실제 테이블에 적용하기 위해 반드시 commit을 실행해야 한다.
    Java와 같은 외부 프로그램을 통해 입력하는 데이터는 자동으로 commit이 되므로 별도의 처리를 하지 않아도 된다.
*/
...
update member set pass='99xx',name='나수정'
    where id='test1';
--더미데이터 입력
insert into member (id, pass, name) values ('commit1', '1234', '테스트');
...
--레코드 검색
select * from member where name like '%o%';
...
/*
아이디가 aaaa는 부모테이블인 member에 존재하지 않으므로 insert문 실행시 에러가 발생한다.
*/
...
/*2개의 테이블이 참조관계에 있다면 테이블 입력시 부모테이블부터 입력해야 한다.
만얃 자식 테이블에 먼저 입력하면 '부모키가 없다'는 에러가 발생한다.*/
...
--위 2개의 쿼리를 병합해서 문자열(아이디)의 첫글자를 제외한 나머지 부분을 *로 마스킹처리하는 쿼리문을 생성한다.
...
begin
    /* 인파라미터로 전달받은 아이디를 마스킹 처리후 반환 */
    retStr := rpad(substr(idStr,1,1), length(idStr), '*');
    return retStr;
end;
/
--우리가 전달한 문자열이 마스킹처리되는지 확인
...
is --변수선언없음
begin
    --인파라미터를 통해 회원정보를 입력하는 insert쿼리문 작성
    insert into member (id, pass, name)
        values (p_id, p_pass, p_name);
    --입력이 정상적으로 처리되면 true를 반환
    if sql%found then
        --입력에 성공한 행의 개수를 얻어와서 아웃파라미터에 저장
        returnVal := sql%rowcount;
        --행의 변화가 생기므로 반드시 커밋해야 한다.
        commit;
    else
        --실패한 경우에는 0을 반환
        returnVal := 0;
    end if;
    --프로기저는 별도의 return없이 아웃파라미터에 값을 할당하기만 하면 된다.
end;
/
--바인드변수 생성

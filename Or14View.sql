...
"테이블 및 뷰가 존재하지 않습니다."라는 오류메시지가 뜬다.*/
select * from member;
...
/*
    ...
    위와같이 변경할 컬럼명을 뷰이름 뒤에 소괄호로 명시해주면 된다.
*/
select * from view_employees;
...
*/
--문제의 조건대로 select문 작성
select employee_id, concat(first_name||' ',last_name) name, email, manager_id
...
*/
/*
1. select문 작성. null이 있는 경우 사칙연산이 되지 않으므로 
nvl()함수를 통해 0으로 변환 후 연산식을 작성해야 한다.*/
select
    employee_id, last_name, 
    ltrim(to_char((salary+(salary*nvl(commission_pct,0)))*12, '999,000')) annual_sal
from employees;
--뷰생성
...
/*
뷰 생성시 연산식이 추가되어 논리적인 컬럼이 생성되는 경우에는 반드시 별칭으로 컬럼명을 명시해야 한다.
...
--1.select문 작성
select 
    employee_id, concat(first_name||' ',last_name), department_id, 
    department_name, to_char(hire_date,'yyyy"년"mm"월"dd"일"'), 
    concat(state_province||'주의 ',city)
from employees

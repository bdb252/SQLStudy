...
alter table pr_emp_const
    add constraints pr_emp_pk primary key (empno);
--데이터사전에서 확인하기
...
--테이블이 생성된 후 제약조건을 부여할 때는 alter명령 사용
...
/*
3. pr_dept_const 테이블에 존재하지 않는 부서의 사원이 배정되지 않도록 외래키 제약조건을 지정하되 
제약조건 이름은 pr_emp_dept_fk 로 지정하시오.
*/
alter table pr_emp_const /* 외래키는 자식테이블에서 생성한다. */
    add constraints pr_emp_dept_fk 

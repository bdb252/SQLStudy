...
/*
concat(문자열1, 문자열2)
: 문자열1, 2를 서로 연결해서 출력. 문자열 부분은 컬럼명을 사용할 수 있음
...
--3단계 : 이름 사이에 공백이 없어 가독성이 떨어지므로 추가
select concat(first_name||' ', last_name), salary, department_id from employees;
--4단계 : 컬럼명이 너무 길게 출력되므로 별칭부여
select concat(first_name||' ', last_name) as full_name, salary, department_id from employees;

/*
initcap(문자열): 문자열의 첫글자만 대문자로 변경
...
형식] lpad('문자열', '전체자리수', '채울문자')
    => 전체 자리수에서 문자열의 길이만큼을 제외한 나머지 좌(우)측 부분을 주어진 문자로 채워준다.
*/
--전체 7글자 중 왼쪽 혹은 오른쪽을 #으로 채운다.
--세번째 인수가 없는 경우는 공백으로 빈공간을 채운다.
...
select
    ...
    , trim(leading '다' from '다람쥐가 나무를 탑니다') /*우측 문자 제거*/
    , trim(trailing '다' from '다람쥐가 나무를 탑니다') /*좌측 문자 제거*/
    , trim(' 다람쥐가 나무를 탑니다 ') /*양쪽의 공백제거*/
from dual;
...
replace() : 문자열을 다른 문자열로 대체할 때 사용한다. 
    ...
    형식1] replace(컬럼명 or 문자열, '변경 대상의 문자', '변경할 문자')
...
/*
instr() : 문자열에서 특정 문자가 위치한 인덱스를 찾아 반환한다.
    형식1] instr(컬럼명, '찾을문자')
        문자열의 처음부터 문자를 찾는다.
    형식2] instr(컬럼명, '찾을문자', 탐색을 시작할 인덱스', '몇번째문자');
        탐색을 시작할 인덱스부터 문자를 찾는다. 단, 찾는 문자중 몇번째에 있는 문자인디 지정할 수 있다.

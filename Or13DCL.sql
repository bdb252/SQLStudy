...
/*
사용자 계정 생성 및 권한 설정
: 해당 작업은 DBA 권한이 있는 최고관리자(sys, system)으로 접속한 후 실행해야 한다.
  새로운 사용자 계정이 생성된 후 접속 및 쿼리실행 테스트는 CMD(명령프롬프트)에서 진행한다.
*/
...
/*
2]생성된 사용자계정에 권한 혹은 역할(Role) 부여
형식] grant 시스템권한1, 시스템권한2 혹은 역할
        to 사용자계정 [with grant 옵션]
        
*/
--접속권한부여
grant create session to test_user1;
/*
    create session 권한 부여 후 접속에는 성공했지만, 테이블은 생성할 수 없다ㅜㅜ;
    -------------------------------------------------------------------------
    CMD>>>
        sqlplus로 접속 성공 후
...
/*
    create table 권한 부여 후 테이블 생성 및 desc명령으로 생성된 테이블의 스키마를 확인할 수 있다.
*/
...
/*
    exit 혹은 quit명령으로 접속을 해제한 후 다시 접속하면 기존암호로는 접속할 수 없다.
    변경한 암호로 다시 접속해야 한다.
*/
...
접속 및 테이블 생성 등의 대부분의 기본작업을 할 수 있는 권한이 포함되어있다.*/
...
/*
    my_role을 통해 권한을 부여했으므로 접속 및 테이블 생성이 정상적으로 실행된다.
*/
...
drop role my_role;
/*
test_user3은 my_role을 통해 권한을 부여받았으므로 해당 롤을 삭제하면 모든 권한이 회수(Revoke)된다.
...
/*
    접속권한 회수 후 접속을 시도할때 비밀번호가 틀리면 '부적합' 에러가 발생하고,
...
※cascade를 명시하면 사용자계정과 관련된 모든 데이터베이스 스키마가 데이터사전으로부터 삭제되고 모든 스키마객체도 물리적으로 삭제된다.
*/
--현재 생성된 사용자계정을 확인할 수 있는 데이터사전

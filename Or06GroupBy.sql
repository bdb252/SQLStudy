...
/*
distinct : 동일한 값이 있는 경우 중복된 레코드를 제거한 후 하나의 레코드만 가져와서 보여준다.
          순수한 하나의 레코드이므로 통계적인 값을 계산할 수 없다.
*/
select distinct job_id from employees;
/*
group by : 동일한 값이 있는 레코드를 하나의 그룹으로 묶어서 인출한다.
    보여지는 것은 하나의 레코드이지만 다수의 레코드가 하나의 그룹으로 묶여진 결과이므로 통계적인 값을 계산할 수 있다.
...
/*
count() 함수를 통해 인출된 행의 개수는 아래와 같이 일반적인 select문으로 검증할 수 있다.
*/
select * from employees where job_id='FI_ACCOUNT';
select * from employees where job_id='PU_CLERK';

/*
group by절이 포함된 select 문의 형식
    select
        컬럼1, 컬럼2 ... 컬럼N 혹은 *(컬럼전체)
    from
        테이블
    where
        조건1 and 조건2 or ... 조건N <- where절에 들어가는 조건은 반드시 물리적으로 존재하는 컬럼이어야함
    group by
        레코드의 그룹화를 위한 컬럼
    having
...
select 
    ...
    trim(to_char(sum(salary),'999,000')) as sumSalary3
from employees;

--10번 부서에서 근무하는 사원들의 급여합계는 얼마인지 인출하시오
select 
    trim(to_char(sum(salary),'$999,000')) as sumSalary3
...
/*
count() 함수를 사용할 때 위 2가지 방법 모두 가능하지만 
*의 사용을 권장한다. 컬럼의 특성 혹은 데이터에 따른 방해를 받지 않으므로 실행속도가 빠르다.
*/
...
/*
group by절 : 여러 개의 레코드를 하나의 그룹으로 묶은 후 그룹화하여 결과를 반환하는 쿼리문.
...
각 레코드별로 서로 다른 이름이 저장되어있으므로 그룹의 조건에 따라 단일 컬럼을 사용할 수 없기 때문이다.*/
...
/*
having : 물리적으로 존재하는 컬럼이 아닌 그룹함수를 통해 논리적으로 생성된 컬럼의 조건을 추가할 때 사용한다.
...
*/
/*같은 부서에서 근무하더라도 담당업무는 다를 수 있으므로 이 문제에서는 group by 절에 2개의 컬럼을 명시해야 한다.
즉 부서로 그룹화한 후 다시 담당업무별로 그룹화한다.*/
select 
    department_id, job_id, count(*), avg(salary)
from employees 
where count(*)>10 /*이 부분에서 에러가 발생함.*/
group by department_id, job_id;
/*담당업무별 사원수는 물리적으로 존재하는 칼럼이 아니므로 where절에 추가하면 에러가 발생한다.
이 경우에는 having절에 조건을 추가해야 한다.
Ex) 급여가 3000인 사원 => 물리적으로 존재하므로 where절에 추가
    평균 급여가 3000인 사원 => 상황에 맞게 논리적으로 만들어낸 결과이므로 having절에 추가*/
--앞에서 발생한 에러는 having절로 조건을 옮기면 해결된다.
select 
    department_id, job_id, count(*), avg(salary)
from employees 
group by department_id, job_id 

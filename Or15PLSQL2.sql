...
declare
    ...
    --필요하다면 선언부에서 변수 선언과 동시에 초기화할 수 있다.
...
begin
    ...
    elsif로 기술해야 한다. 또한 중괄호대신 then과 end if를 사용한다.*/    
    if emp_dept = 50 then
        dept_name := 'Shipping';
...
begin
    ...
    /*
    case-when문이 if문과 다른 점은 할당할 변수를 먼저 선언한 후 문장 내에서 조건을 판단하여 하나의 값을 할당하는 방식이다.
    따라서 변수를 중복해서 기술하지 않아도 된다. */
    dept_name :=
        case emp_dept
            when 50 then 'Shipping'
            when 60 then 'IT'
            when 70 then 'Shipping'
            when 80 then 'Public Relations'
...
/*
반복문1 : Basic loop문
    Java의 do~while문과 같이 조건 체크없이 일단 loop로 진입한 후 탈출조건이 될때까지 반복한다.
    탈출시에는 exit를 사용한다.
*/
...
/*
반복문2 : while문
    Basic loop와는 다르게 조건을 먼저 확인한 후 실행한다. 즉 조건에 맞지 않는다면 한번도 실행되지 않을 수 있다.
...
따라서 특별한 이유가 없다면 선언부(declare)를 기술하지 않아도 된다.
*/
--예제15] 제어문(반복문 : for) 
declare
    --선언부에 선언한 변수가 없다.
...
/*
커서(Cursor)
:select 쿼리에 의해 여러행이 반환되는 경우 각 행에 접근하기 위한 객체
선언방법]
    Cursor 커서명 Is
        select쿼리문. 단 into절이 없는 형태로 기술한다.
    Open 커서명
        :쿼리를 실행하라는 의미. 즉 Open할때 Cursor선언시 select문이 실행되어 결과셋을 얻게 된다.
        Cursor는 그 결과셋의 첫번째 행에 위치하게 된다.
    Fetch~Into~
        :결과셋에서 하나의 행을 읽어들이는 작업으로 결과셋의 인출(Fetch)후에 Cursor는 다음 행으로 이동한다.
    Close 커서명
        :커서 닫기로 결과셋의 자원을 반납한다. select문장이 모두 처리된 후 Cursor를 닫아준다.
Cursor의 속성
    %Found : 가장 최근에 인출(Fetch)된 행을 반환하면 True, 아니면 False를 반환한다.
    %NotFound : %Found의 반대의 값을 반환한다.
    %RowCount : 지금까지 반환된 행의 개수를 반환한다.
*/
--예제17] Cursor
/*
...
declare
    ...
    /* 커서선언 : 부서테이블의 모든 레코드를 조회하는 select문으로 into절이 없는 형태로 작성
      쿼리의 실행결과가 cur1에 저장된다.*/
    cursor cur1 is
        select
            department_id, department_name, location_id
        from departments;
begin
    /* 해당 쿼리문을 수행해서 결과셋(ResultSet)을 얻어온다.
    결과셋이란 쿼리문을 실행한 후 반환되는 레코드의 결과를 말한다.*/
    open cur1;
    
    /* 결과셋의 개수는 조건에 따라 다를 수 있으므로 Basic loop를 통해 반복 인출한다.*/
    loop
        fetch cur1 into 

...
-시퀀스는 테이블 생성 후 별도로 만들어진다. 따라서 시퀀스는 테이블과 독립적으로 생성되고 저장된다.
...
/*
시퀀스 생성 후 최초 실행 시에는 currval을 싱행할 수 없어 에러가 발생함.
nextval을 먼저 실행해서 시퀀스를 얻어온 후 실행해야 한다.
*/
select seq_serial_num.currval from dual;
/*
다음 입력할 시퀀스를 반환한다. 실행할 때마다 설정한 증가치만큼 증가된 값이 반환된다.
*/
select seq_serial_num.nextval from dual;
/*
시퀀스의 nextval을 사용하면 계속 새로운 값을 반환하므로 아래와 같이 insert문에 사용할 수 있다.
또한 같은 쿼리문을 여러번 실행하더라도 문제없이 입력된다.*/
...
즉 PK컬럼에 사용할 시퀀스는 cycle옵션을 사용하지 않아야 한다.*/
...
alter sequence seq_serial_num
    increment by 1
    minvalue 9  /* 최소값은 기존에 설정된 값보다 크면 에러 발생함 */

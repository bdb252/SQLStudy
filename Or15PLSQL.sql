...
: 일반 프로그래밍 언어에서 가지고 있는 요소를 모두 가지고 있으며 DB업무를 처리하기 위해 최적화된 언어
*/
...
--예제1] PL/SQL 맛보기
--화면상에 내용을 출력하고 싶을 때 on으로 설정한다. off면 출력되지 않는다.
set serveroutput on;

declare --선언부 : 주로 변수를 선언한다. (PL/SQL시작부분)
...
declare
    ...
    '이름'의 경우 2개의 컬럼이 합쳐진 상태이므로 조금 더 넉넉한 크기로 설정해주는 것이 좋다*/
    empName varchar2(50);
...
/
/*
    PL/SQL 문장의 끝에는 반드시 /을 붙여야하는데, 만약 없으면 호스트환경으로 빠져나오지 못한다.
    즉 PL/SQL문장을 탈출하기 위해 필요하다.
    호스트환경이란 쿼리문을 입력하기 위한 SQL> 프롬프트가 있는 상태를 말한다.
*/

--예제3] 참조변수1(하나의 컬럼 참조) 
/*
참조변수 : 특정 테이블의 컬럼을 참조하는 변수로써 동일한 자료형과 크기로 선언하고 싶을 때 사용한다.
    형식] 테이블명.컬럼명%type
        =>테이블에서 '하나'의 컬럼만을 참조한다.
*/
/*
시나리오] 부서번호 10인 사원의 사원번호, 급여, 부서번호를 가져와서 아래 변수에 대입후 
화면상에 출력하는 PL/SQL문을 작성하시오. 단, 변수는 기존테이블의 자료형을 참조하는 '참조변수'로 선언하시오.
*/
...
declare
    ...
    empNo employees.employee_id%type; --NUMBER(6,0)으로 선언
    empSal employees.salary%type;     --NUMBER(8,2)와 같이 선언됨
    depId employees.department_id%type;--NUMBER(4,0)
begin
...
begin
    ...
    /*
    emp_row에는 전체컬럼의 정보가 저장되므로 출력시 변수명.컬럼명 형태로 기술해야 한다.*/
...
/*
복합변수
: class를 정의하듯 필요한 자료형을 하나로 묶어 새로운 자료형을 만든 후 생성하는 변수를 말한다.
형식] 
...
declare
    ...
    /* 앞에서 정의한 자료형을 통해 복합변수를 선언한다. 이 변수는 3개의 값을 저장할 수 있다. */
    record3 emp_3type;
begin
    --select절에서 인출한 3개의 값을 into 절에서 복합변수에 할당
    select employee_id, first_name||' '||last_name, job_id
        into record3
    from employees where employee_id=100;
    --출력
...
치환연산자 : PL/SQL에서 사용자로부터 데이터를 입력받을 때 사용하는 연산자로 
    변수앞에 &를 붙여주면 된다. 실행 시 입력창이 뜬다. */
--시나리오] 앞에서 작성한 프로그램을 치환연산자로 사원번호를 입력받을 수 있도록 수정하시오.
declare
    type empTypes is record(
        emp_id employees.employee_id%type,
        emp_name varchar2(50),
...
/*
바인드변수
: 호스트 환경에서 선언된 변수로써 비 PL/SQL 변수이다.
  호스트 환경은 SQL> 프롬프트가 있는 상태로 명령어(쿼리문)를 입력할 수 있는 상태를 말한다.
...
begin
    --바인드변수는 일반변수와의 구분을 위해 :(콜론)을 앞에 추가한다.
    :return_var :=999;
...
print return_var;
/*
    호스트 환경에서 바인드변수를 출력할 때는 print를 사용한다.
    CMD 에서는 개별적으로 실행해도 문제가 없지만, 
    디벨로퍼에서는 바인드 변수로부터 마지막 print까지 묶은 후 실행해야 결과가 제대로 출력된다.
*/

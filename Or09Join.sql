...
파일명 : Or09Join.sql
테이블 조인
설명 : 2개 이상의 테이블을 동시에 참조하여 데이터를 인출해야 할 때 사용하는 SQL문
***/
...
- 일반적으로 기본키(Primary Key)와 외래키(Foriegn Key)를 사용하여 join하는 경우가 대부분이다.
- 2개의 테이블에 동일한 이름의 컬럼이 존재해야 하고, 이 경우 '테이블명.컬럼명'과 같이 기술한다.
...
/*위의 첫번째 쿼리문을 실행하면 열의 정의가 애매하다는 에러가 발생한다.
부서번호를 뜻하는 department_id가 양쪽 테이블 모두에 존재하므로 어떤 테이블에서 가져와 인출해야 할지 명시해야 한다.*/
...
즉 inner join은 조인한 테이블 양쪽에 만족되는 레코드를 대상으로 인출하게된다.*/
...
select * from employees where department_id=10; --1명확인
select * from employees where department_id=30; --6명확인
...
/*
2] outer join(외부조인)
outer join은 inner join과는 달리 두 테이블간의 조인조건이 정확히 일치하지 않아도 기준이 되는 테이블에서 레코드를 인출하는 방식이다.
...
부서가 지정되지 않은 사원까지 인출되리 때문인데, 이 경우 부서쪽에 레코드가 없으므로 null이 출력된다.*/
...
결과는 모두 19개 인출됨.*/
select 
    employee_id, first_name, last_name, hire_date, nvl(department_name, '<부서없음>') 
from employees E left outer join departments D
    on E.department_id=D.department_id
    where to_char(hire_date,'yyyy')='2007';

--내쿼리
select employee_id, first_name, last_name, nvl(department_name, '<부서없음>') --, hire_date --디버깅용
from employees Em
    left outer join departments De
        on Em.department_id=De.department_id
...
/* 오라클 방식으로 변경 시 outer join 연산자인 (+)를 생략하면 inner join이 되므로 작성에 주의해야 한다. */
...
--2.사원입장의 매니져아이디와 매니져입장의 사원아이디를 조인한다.
...
select 
    ...
    empClerk.first_name || ' ' || empClerk.last_name "사원의이름",
...
--2.킴벌리의 입사일보다 늦은(큰) 데이터를 조건으로 추가
select 
...
where 
    ...
    and Clerk.hire_date>Kim.hire_date;
...
전제로 작성되기 때문에 굳이 별칭을 사용할 필요가 없기 때문이다.
...
--지역일련번호를 통해 해당 지역의 부서를 확인(결과 : 50)
...
/*
2005년에 입사했고, 50번 부서(Shipping)에 근무하는 사원의 정보를 인출하는 inner join 쿼리문을 작성한다.*/
select 
...
where to_char(hire_date,'yyyy')='2005' and
    ...
    lower(city)='south san francisco';
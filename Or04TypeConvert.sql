...
/*
sysdate : 현재날짜와 시간을 초단위로 반환해준다. 주로 게시판이나 회원가입시 날짜를 입력하기 위해 사용된다.
    오라클의 기본 서식이 xx/xx/xx이므로 출력시 시간은 표시되지 않지만, 서식문자를 통해 초단위까지 표현할 수 있다.
*/
...
select to_char(sysdate, '오늘은 YYYY년MM월DD일입니다.') from dual;
/*
-(하이픈), /(슬러쉬)와 같은 특수기호 외에는 서식으로 인식하지 못하므로 이를 제외한 나머지 문자열은 "으로 묶어줘야 한다.
...
*/
select
    to_char(123,'0000'), to_char(123, '9999'), trim(to_char(123, '9999'))
    from dual;

select 12345,
    to_char(12345,'0000'), 
    to_char(123, '999,9999'), ltrim(to_char(12345, '999,999'))
    from dual;

/*
숫자에 세자리마다 컴마 표시하기
: 자리수가 확실히 보장된다면 0을 사용하고, 자리수가 확실하지 않은 경우에는 9를 사용한다.
 대신 공백은 trim으로 제거하면 된다.
*/
select 12345,
    to_char(12345,'0000'), 
    to_char(123, '999,9999'), 
...
*/
--문자열 사이에 콤마가 포함되어있으므로 숫자로 변환할 수 없어 에러 발생함
...
/*
to_date()
    문자열 데이터를 날짜형식으로 변환해서 출력한다. 기본서식은 년/월/일 순으로 지정된다.
*/
...
/*
아래와 같이 날짜포맷이 년-월-일 이 아닌 경우에는 오라클이 인식하지 못하므로 에러가 발생한다.
...
from dual;
/*
방법2 : 날짜와 시간의 서식을 적용한다. 특히 시가능ㄴ 15시이므로 24시간제인 서식을 이용해야 한다.
*/
select
    to_char(to_date('2020-10-14 15:30:21', 'yyyy-mm-dd hh24:mi:ss')+1,'yyyy-mm-dd hh24:mi:ss')
from dual;
...
select empno, ename, job, mgr, to_char(hiredate, 'yy/mm/dy')
from emp;
...
select floor(sysdate - to_date('01-01-2025', 'dd-mm-yyyy')) as day2025
from dual;

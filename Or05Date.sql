...
파일명 : Or05Date.sql
날짜함수
설명 : 년, 월, 일, 시, 분초의 포맷으로 날짜형식을 지정하거나 날짜를 계산할 때 사용하는 함수
***/
/*
months_between() : 현재날짜와 기준날짜 사이이 개월수를 반환한다.
    형식] months_between(현재날짜, 기준날짜[과거]);
*/
select 
    months_between(sysdate, '2024-01-01'),
    ceil(months_between(sysdate, '2024-01-01')) "올림처리",
    floor(months_between(sysdate, '2024-01-01')) "내림처리"
from dual;
...
*/
/*
"2020년01월01일" 이 문자열은 날짜의 형식으로 인식되지 않으므로 to_date를 통해 날짜형식으로 변환해야 한다.
또한 날짜서식 사이에 한글은 허용되지 않으므로 더블쿼테이션으로 감싸줘야 한다.
*/
select
    months_between(sysdate, to_date('2020년01월01일', 'yyyy"년"mm"월"dd"일')),
    ceil(months_between(sysdate, to_date('2020년01월01일', 'yyyy"년"mm"월"dd"일'))) "개월수올림",
    floor(months_between(sysdate, to_date('2020년01월01일', 'yyyy"년"mm"월"dd"일'))) "개월수내림"
from dual;
